...
"""
知乎专栏文章爬取脚本
用法: python scrape_zhihu.py <知乎专栏链接> [选项]

示例:
  python scrape_zhihu.py https://zhuanlan.zhihu.com/p/678041376
  python scrape_zhihu.py https://zhuanlan.zhihu.com/p/678041376 --tags 广州,新能源
  python scrape_zhihu.py https://zhuanlan.zhihu.com/p/678041376 --date 2024-02-20 --slug car-transfer

依赖安装:
  pip install Pillow requests
  npm install puppeteer-core   （在 scripts/ 目录下）
...
RENDERER_SCRIPT = SCRIPT_DIR / "_zhihu_renderer.mjs"


# ── 调用 Node.js 渲染器抓取知乎 ────────────────────────────────────────

def scrape_zhihu(url: str) -> dict:
    """调用 _zhihu_renderer.mjs 渲染页面，返回结构化数据。"""
    ...
    if not data.get("items"):
        print("错误：未能提取到文章内容，可能页面未正确加载或被反爬拦截。")
        sys.exit(1)
...
def _items_to_markdown(items: list[dict]) -> str:
    ...
    for item in items:
        ...
        if t in ("h2", "h3", "h4"):
            ...
            # 占位符，后续由 download_and_convert_images 替换
            src = item.get("src", "")
...
def download_and_convert_images(markdown: str, slug: str) -> tuple[str, list[str]]:
    ...
    for i, m in enumerate(matches, 1):
        ...
        print(f"  下载图片 {i}/{len(matches)}: {filename}.webp")
        try:
            resp = requests.get(
                img_url,
                headers={
...
def main():
    ...
    parser.add_argument("url", help="知乎专栏文章链接，如 https://zhuanlan.zhihu.com/p/678041376")
    parser.add_argument("--slug", help="文件名 slug（默认自动从标题生成）")
    parser.add_argument("--date", help="发布日期，如 2024-02-20（默认今天）")
    parser.add_argument("--tags", help="标签，逗号分隔，如 广州,新能源")
